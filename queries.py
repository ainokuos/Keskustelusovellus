...
def get_answers(question_id):
    sql = "SELECT C.choice, count(A.choice_id) FROM choices C, answers A" \
          " WHERE A.choice_id = C.id AND C.question_id =:question_id" \
          " GROUP BY C.id, C.choice, C.question_id"
    result = db.session.execute(sql, {"question_id":question_id}).fetchall()
    return result
...
def answered(question_id):
    sql = "SELECT A.user_id FROM answers A, questions Q, choices C" \
          " WHERE Q.id = C.question_id AND C.id = A.choice_id AND Q.id =:question_id"
...
def get_sum(question_id):
    sql = "SELECT COUNT(A.id) FROM answers A, choices C" \
          " WHERE A.choice_id=C.id AND C.question_id=:question_id;"
